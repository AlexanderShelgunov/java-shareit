...
public class ItemControllerTests {
    ...
    @Test
    void testCreateItemWithoutUserIdHeader() throws Exception {
        ItemInputDto itemInputDto = ItemMapper.toItemDto(item);
        when(itemService.createItem(itemInputDto, 1L)).thenReturn(itemInputDto);
    ...
    @Test
    void testUpdateItemNameByNonOwnerShouldThrowException() throws Exception {
        ItemInputDto itemDto = ItemMapper.toItemDto(item);
        ItemInputDto itemInputDto = ItemInputDto.builder().name("NameUpdated").build();
        ...
        when(itemService.updateItem(itemDto.getId(), itemInputDto, 11L))
                .thenThrow(new ForbiddenException("Редактировать вещь может только её владелец."));
    ...
    @Test
    void testUpdateItemWithoutUserIdHeader() throws Exception {
        ItemInputDto itemDto = ItemMapper.toItemDto(item);
        ItemInputDto itemInputDto = ItemInputDto.builder().name("NameUpdated").build();
        ItemInputDto itemOutput = ItemInputDto.builder()
                .id(1L)
    ...
    @Test
    void testGetAllItemsByOwnerWithWrongParam() throws Exception {
            mvc.perform(get("/items")
                    .header("X-Sharer-User-Id", 1L)
                    .param("from", "-2")
                    .param("size", "2"))
    ...
    @Test
    void testSearchItemsWithWrongParam() throws Exception {
            mvc.perform(get("/items/search")
                    .header("X-Sharer-User-Id", 1L)
                    .param("text", "Item")
                    .param("from", "0")
